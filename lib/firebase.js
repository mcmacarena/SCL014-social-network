...
export const forgetPassword = () => {
  const emailForgot = document.querySelector('#forgetPasswordEmail').value;
  const messageSend = document.querySelector('#textForgetPassword');

  firebase.auth().sendPasswordResetEmail(emailForgot)
    .then(() => {
      messageSend.innerHTML = 'Correo para reestablecer contraseña ha sido enviado, por favor revisa tu bandeja';
    })
    .catch((error) => {
      messageSend.innerHTML = 'Correo no registrado o invalido, intentálo nuevamente';
