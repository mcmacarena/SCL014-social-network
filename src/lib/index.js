...
const redimencionarImagen = (base64, maxWidth, maxHeight) => {
// function redimencionarImagen(srcData, width, height) {
  console.log('Entro en redimencionar');
  console.log(base64);
  ...
  const CopiaCambas = document.createElement('canvas');
  const copyContext = CopiaCambas.getContext('2d');
  const ImgenTemporal = new Image();
  ImgenTemporal.src = base64;
  let ratio = 1;
  if (ImgenTemporal.width > maxWidth) ratio = maxWidth / ImgenTemporal.width;
  else if (ImgenTemporal.height > maxHeight) ratio = maxHeight / ImgenTemporal.height;
  ...
  console.log('fearia de impresiones');
  console.log(xStart);
  console.log(yStart);
  console.log(newWidth);
  console.log(newHeight);
  console.log(ctx);
  ctx.drawImage(imageObj, xStart, yStart, newWidth, newWidth);
  console.log(ctx);
...
export const photoProfileUpload = () => {
  document.querySelector('#photoDog').addEventListener('change', (e) => {
    console.log('Entto en photoprofile upload');
    const photoDogLocal = e.target.files[0];
    console.log('**********************1*******************');
    console.log(photoDogLocal);
    console.log('**********************1*******************');

    console.log('**********************2.1*******************');
    console.log(getBase64(photoDogLocal));
    console.log('**********************2.1*******************');
    console.log('**********************2.2*******************');
    const imagenScaled = redimencionarImagen(getBase64(photoDogLocal), 500, 500);
    console.log(imagenScaled);
    console.log('**********************2.2*******************');
    console.log('**********************3*******************');
    console.log(dataURItoBlob(imagenScaled));
    console.log('**********************3*******************');
    uploadProfilePhoto(dataURItoBlob(imagenScaled));
  });
};
